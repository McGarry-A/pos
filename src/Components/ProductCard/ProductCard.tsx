...
const ProductCard = ({ id, title, price, icon }: Props) => {
  ...
  return (
    <button
      className="flex justify-center flex-col items-center max-w-xl rounded-md border cursor-pointer  shadow-sm hover:shadow-md bg-white p-3"
      key={id}
      onClick={() => handleClick()}
    ...
    >
      <div
        className={`ml-auto ${
          showDecrement === true ? "visible" : "invisible"
        }`}
